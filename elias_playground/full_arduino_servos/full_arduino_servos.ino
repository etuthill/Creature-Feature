...
int leftTargetPos = 85;
int rightTargetPos = 85;
int backTargetPos = 95;

const unsigned long servoInterval = 20;   // ms between updates
const float speedDegPerSec = 90.0;         // degrees per second
...
void moveServoSmooth(
  ...
  if (now - lastUpdateTime < servoInterval) return;
  lastUpdateTime = now;
  ...
  if (currentPos < targetPos)
    currentPos = min((float)currentPos + step, (float)targetPos);
  else
    currentPos = max((float)currentPos - step, (float)targetPos);
